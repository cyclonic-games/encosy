...
Object.defineProperty(module.exports, '__instances__', {
    get () {
        return store.get('entities').values();
    }
});

Object.defineProperty(module.exports, 'serialize', {
    value (entity) {
        const entities = Array.from(store.get('entities').keys());
        const components = { };
        const id = entity.valueOf();
        const { kind = 'unknown' } = entities.find(x => store.get('entities').get(x).has(entity)) || { };
